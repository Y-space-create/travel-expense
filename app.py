...
if db["records"]:
    ...
    with col1:
        st.write("** 按人员**")
        st.dataframe(df.groupby("person")["amount"].sum().reset_index().sort_values("amount",  ascending=False))
    with col2:
        st.write("** 按分类**")
        st.dataframe(df.groupby("category")["amount"].sum().reset_index().sort_values("amount",  ascending=False))
    total = df["amount"].sum()
